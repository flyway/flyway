...
call mvn install:install-file -DgroupId=%GROUP_ID% -Dversion=%VERSION% -Dfile=flyway-core\flyway-core-%VERSION%.jar                             -DartifactId=flyway-core          -Dpackaging=jar    -DpomFile=flyway-core\pom.xml          -Djavadoc=flyway-core\flyway-core-%VERSION%-javadoc.jar || goto :error
call mvn install:install-file -DgroupId=%GROUP_ID% -Dversion=%VERSION% -Dfile=flyway-gradle-plugin\flyway-gradle-plugin-%VERSION%.jar           -DartifactId=flyway-gradle-plugin -Dpackaging=jar    -DpomFile=flyway-gradle-plugin\pom.xml -Djavadoc=flyway-gradle-plugin\flyway-gradle-plugin-%VERSION%-javadoc.jar || goto :error
call mvn install:install-file -DgroupId=%GROUP_ID% -Dversion=%VERSION% -Dfile=flyway-maven-plugin\flyway-maven-plugin-%VERSION%.jar             -DartifactId=flyway-maven-plugin  -Dpackaging=jar    -DpomFile=flyway-maven-plugin\pom.xml  -Djavadoc=flyway-maven-plugin\flyway-maven-plugin-%VERSION%-javadoc.jar || goto :error
call mvn install:install-file -DgroupId=%GROUP_ID% -Dversion=%VERSION% -Dfile=flyway-commandline\flyway-commandline-%VERSION%.jar               -DartifactId=flyway-commandline   -Dpackaging=jar    -DpomFile=flyway-commandline\pom.xml   -Djavadoc=flyway-commandline\flyway-commandline-%VERSION%-javadoc.jar || goto :error
