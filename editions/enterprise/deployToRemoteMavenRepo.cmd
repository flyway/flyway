...
call mvn -f pom.xml deploy:deploy-file -DrepositoryId=%REPO_ID% -Durl=%REPO_URL% -DgroupId=%GROUP_ID% -Dversion=%VERSION% -Dfile=flyway-commandline/flyway-commandline-%VERSION%.jar               -DartifactId=flyway-commandline   -Dpackaging=jar    -DupdateReleaseInfo=true -DpomFile=flyway-commandline/pom.xml   -Djavadoc=flyway-commandline/flyway-commandline-%VERSION%-javadoc.jar     -Dsources=flyway-commandline/flyway-commandline-%VERSION%-sources.jar     || goto :error
call mvn -f pom.xml deploy:deploy-file -DrepositoryId=%REPO_ID% -Durl=%REPO_URL% -DgroupId=%GROUP_ID% -Dversion=%VERSION% -Dfile=flyway-commandline/flyway-commandline-%VERSION%.zip               -DartifactId=flyway-commandline   -Dpackaging=zip    -DgeneratePom=false || goto :error
call mvn -f pom.xml deploy:deploy-file -DrepositoryId=%REPO_ID% -Durl=%REPO_URL% -DgroupId=%GROUP_ID% -Dversion=%VERSION% -Dfile=flyway-commandline/flyway-commandline-%VERSION%.tar.gz            -DartifactId=flyway-commandline   -Dpackaging=tar.gz -DgeneratePom=false || goto :error
