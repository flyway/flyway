...
public class ValidatePatternUtils {
    public static boolean isPendingIgnored(final ValidatePattern[] ignorePatterns) {
    ...
    public static boolean isIgnoredIgnored(final ValidatePattern[] ignorePatterns) {
    ...
    public static boolean isMissingIgnored(final ValidatePattern[] ignorePatterns) {
        return Arrays.stream(ignorePatterns).anyMatch(p -> p.equals(ValidatePattern.fromPattern("*:missing"))) || (
            Arrays.stream(ignorePatterns).anyMatch(p -> p.matchesMigration(true, MigrationState.MISSING_SUCCESS))
                ...
                .anyMatch(p -> p.matchesMigration(false, MigrationState.MISSING_SUCCESS)));
    }

    public static boolean isFutureIgnored(final ValidatePattern[] ignorePatterns) {
