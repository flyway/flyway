 ...
 */
public class SinoDBSchema extends Schema<SinoDBDatabase, SinoDBTable> {
    /**
     ...
     */
    InformixSchema(JdbcTemplate jdbcTemplate, SinoDBDatabase database, String name) {
    ...
    private SinoDBTable[] findTables(String sqlQuery, String... params) throws SQLException {
        List<String> tableNames = jdbcTemplate.queryForStringList(sqlQuery, params);
        SinoDBTable[] tables = new SinoDBTable[tableNames.size()];
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new SinoDBTable(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }

    @Override
    protected SinoDBTable[] doAllTables() throws SQLException {
