...
public class RedshiftSchema extends Schema<RedshiftDbSupport> {
     ...
     */
    RedshiftSchema(JdbcTemplate jdbcTemplate, RedshiftDbSupport dbSupport, String name) {
    ...
    private List<String> generateDropStatementsForRoutines() throws SQLException {
        ...
        for (Map<String, String> row : rows) {
            statements.add("DROP FUNCTION " + dbSupport.quote(name, row.get("proname")) + "(" + row.get("args") + ") CASCADE");
        }
        return statements;
    }
    ...
    protected Type getType(String typeName) {
        return new RedshiftType(jdbcTemplate, dbSupport, this, typeName);
    }
}