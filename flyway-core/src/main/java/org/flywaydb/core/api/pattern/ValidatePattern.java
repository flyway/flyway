...
public class ValidatePattern {
    ...
    public static ValidatePattern fromPattern(final String pattern) {
        if (pattern == null) {
        ...
        if (patternParts.length != 2) {
            throw new FlywayException("Invalid pattern '"
                + pattern
                + "'. Pattern must be of the form <migration_type>:<migration_state> "
                + "See "
                + FlywayDbWebsiteLinks.IGNORE_MIGRATION_PATTERNS
                + " for full details");
        }

        final String migrationType = patternParts[0].trim().toLowerCase();
        final String migrationState = patternParts[1].trim().toLowerCase();

        if (!validMigrationTypes.contains(migrationType)) {
            throw new FlywayException("Invalid migration type '"
                + patternParts[0]
                + "'. Valid types are: "
                + validMigrationTypes);
        }
        ...
        if (!validMigrationStates.contains(migrationState)) {
            throw new FlywayException("Invalid migration state '"
                + patternParts[1]
                + "'. Valid states are: "
                + validMigrationStates);
        }

        return new ValidatePattern(migrationType, migrationState, pattern);
    }

    public boolean matchesMigration(final boolean isVersioned, final MigrationState state) {
    ...
    public boolean equals(final Object o) {
        ...
        final ValidatePattern that = (ValidatePattern) o;
