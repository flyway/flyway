...
public class JdbcMigrationResolverSmallTest {
    ...
    public void broken() {
        new JdbcMigrationResolver(Thread.currentThread().getContextClassLoader(), new Location("org/flywaydb/core/internal/resolver/jdbc/error"), "V", "__").resolveMigrations();
    }

    @Test
    public void resolveMigrations() {
        JdbcMigrationResolver jdbcMigrationResolver =
                new JdbcMigrationResolver(Thread.currentThread().getContextClassLoader(), new Location("org/flywaydb/core/internal/resolver/jdbc/dummy"), "V", "__");
    ...
    public void conventionOverConfiguration() {
        JdbcMigrationResolver jdbcMigrationResolver = new JdbcMigrationResolver(Thread.currentThread().getContextClassLoader(), null, "V", "__");
