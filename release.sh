...
if [ "$FLYWAY_MAVEN_AUTO_RELEASE" == "true" ]; then
  ...
  mvn -s "$SETTINGS_FILE" -P$PROFILE org.sonatype.plugins:nexus-staging-maven-plugin:1.6.8:rc-close -DserverId=$NEXUS_ID -DnexusUrl=$NEXUS_URL -DstagingRepositoryId=$STAGING_REPOSITORY_ID -DstagingDescription="Automated closing of Staging Repository"
  mvn -s "$SETTINGS_FILE" -P$PROFILE org.sonatype.plugins:nexus-staging-maven-plugin:1.6.8:rc-release -DserverId=$NEXUS_ID -DnexusUrl=$NEXUS_URL -DstagingRepositoryId=$STAGING_REPOSITORY_ID -DstagingDescription="Automated release of Staging Repository"
fi
