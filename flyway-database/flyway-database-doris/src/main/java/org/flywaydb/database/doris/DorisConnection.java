...
public class DorisConnection extends Connection<DorisDatabase> {
    ...
    private boolean hasUserVariableResetCapability() {
        /*if (database.getVersion().isAtLeast("5.7")) {
    ...
    protected Schema doGetCurrentSchema() throws SQLException {
        ...
        // #2206: MySQL can have URLs where no current schema is set, so we must handle this case explicitly.
    ...
    public <T> T lock(Table table, Callable<T> callable) {
        //doris does not support "LOCK TABLES" or SELECT ... FOR UPDATE,so just execute the callable directly
