...
public class HSQLDBParser extends Parser {
    ...
    public HSQLDBParser(final Configuration configuration, final ParsingContext parsingContext) {
        super(configuration, parsingContext, 2);
    }

    @Override
    protected Set<String> getValidKeywords() {
        return new HashSet<>(Arrays.asList("ABS",
            "ALL",
            "ALLOCATE",
            "ALTER",
            "AND",
    ...
    protected int getLastKeywordIndex(final List<Token> tokens, final int endIndex) {
        for (int i = endIndex - 1; i >= 0; i--) {
            final String tokenText = tokens.get(i).getText();
    ...
    protected boolean shouldAdjustBlockDepth(final ParserContext context, final List<Token> tokens, final Token token) {
        ...
        if (tokenText != null
            && getValidKeywords().contains(tokenText.toUpperCase(Locale.ENGLISH))
    ...
    protected void adjustBlockDepth(final ParserContext context,
        ...
        if ("BEGIN".equalsIgnoreCase(keywordText) || ((("IF".equalsIgnoreCase(keywordText)
            ...
            || "FOR".equalsIgnoreCase(keywordText) || "CASE".equalsIgnoreCase(keywordText) || "WHILE".equalsIgnoreCase(
            keywordText) || "LOOP".equalsIgnoreCase(keywordText) || "REPEAT".equalsIgnoreCase(keywordText))
