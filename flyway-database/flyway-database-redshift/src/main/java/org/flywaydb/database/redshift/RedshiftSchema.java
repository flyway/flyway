...
public class RedshiftSchema extends Schema<RedshiftDatabase, RedshiftTable> {
     ...
     */
    RedshiftSchema(final JdbcTemplate jdbcTemplate, final RedshiftDatabase database, final String name) {
    ...
    protected void doClean() throws SQLException {
        ...
        for (final String statement : generateDropStatementsForRoutines('a', "FUNCTION", " CASCADE")) {
            jdbcTemplate.execute(statement);
        }
        for (final String statement : generateDropStatementsForRoutines('f', "FUNCTION", " CASCADE")) {
            jdbcTemplate.execute(statement);
        }
        for (final String statement : generateDropStatementsForRoutines('p', "PROCEDURE", "")) {
            jdbcTemplate.execute(statement);
        }
    }
     ...
     */
    private List<String> generateDropStatementsForRoutines(final char kind, final String objType, final String cascade)
        throws SQLException {
        final List<Map<String, String>> rows = jdbcTemplate.queryForList(
            ...
            "SELECT proname, oidvectortypes(proargtypes) AS args "
                ...
                + "WHERE pg_proc_info.proisagg = false AND pg_proc_info.prokind = '"
                + kind
                + "' "
                + "AND ns.nspname = ? AND dep.objid IS NULL", name);

        final List<String> statements = new ArrayList<>();
        for (final Map<String, String> row : rows) {
            statements.add("DROP "
                + objType
                + database.quote(name, row.get("proname"))
                + "("
                + row.get("args")
                + ") "
                + cascade);
    ...
    protected RedshiftTable[] doAllTables() throws SQLException {
        ...
        final RedshiftTable[] tables = new RedshiftTable[tableNames.size()];
