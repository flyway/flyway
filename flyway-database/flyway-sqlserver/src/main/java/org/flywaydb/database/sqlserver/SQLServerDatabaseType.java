...
public class SQLServerDatabaseType extends BaseDatabaseType {
    ...
    public boolean handlesJDBCUrl(final String url) {
        return isSecretManagerUrl(url, "sqlserver") || url.startsWith("jdbc:sqlserver:") || (supportsJTDS()
            && url.startsWith("jdbc:jtds:")) || url.startsWith("jdbc:p6spy:sqlserver:") || (supportsJTDS()
            && url.startsWith("jdbc:p6spy:jtds:"));
    }
    ...
    public void setConfigConnectionProps(final Configuration config,
        ...
        if (config != null) {
            final SQLServerConfigurationExtension configurationExtension = config.getPluginRegister()
                .getExact(SQLServerConfigurationExtension.class);
            final SQLServerKerberosSupport kerberosSupport = config.getPluginRegister()
                .getInstanceOf(SQLServerKerberosSupport.class);
            kerberosSupport.configureKerberos(config, configurationExtension);
        }
    }
}
