...
public class FabricDataWarehouseTable extends SQLServerTable {
    ...
    FabricDataWarehouseTable(final JdbcTemplate jdbcTemplate,
    ...
    protected void doLock() throws SQLException {
        ...
        final Timestamp currentDateTime = new Timestamp(cal.getTime().getTime());

        final String updateLockStatement = "UPDATE "
            + this
            + " SET installed_on = '"
            + currentDateTime
            + "' WHERE version = '?' AND description = 'flyway-lock'";
        final String deleteExpiredLockStatement = "DELETE FROM "
            + this
            + " WHERE description = 'flyway-lock' AND installed_on < '?'";
