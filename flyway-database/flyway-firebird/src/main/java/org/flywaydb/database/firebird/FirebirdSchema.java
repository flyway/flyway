...
public class FirebirdSchema extends Schema<FirebirdDatabase, FirebirdTable> {
    ...
    public FirebirdSchema(final JdbcTemplate jdbcTemplate, final FirebirdDatabase database, final String name) {
    ...
    protected void doClean() throws SQLException {
        ...
        for (final String dropPackageStmt : generateDropPackageStatements()) {
            jdbcTemplate.execute(dropPackageStmt);
        }
        for (final String dropProcedureStmt : generateDropProcedureStatements()) {
            jdbcTemplate.execute(dropProcedureStmt);
        }
        for (final String dropViewStmt : generateDropViewStatements()) {
            jdbcTemplate.execute(dropViewStmt);
        }

        for (final String dropConstraintStmt : generateDropConstraintStatements()) {
        ...
        for (final String dropTriggerStmt : generateDropTriggerStatements()) {
            jdbcTemplate.execute(dropTriggerStmt);
        }
        for (final String dropFunctionStmt : generateDropFunctionStatements()) {
            jdbcTemplate.execute(dropFunctionStmt);
        }
        for (final String dropSequenceStmt : generateDropSequenceStatements()) {
            jdbcTemplate.execute(dropSequenceStmt);
        }
        for (final String dropExceptionStmt : generateDropExceptionStatements()) {
            jdbcTemplate.execute(dropExceptionStmt);
        }
        for (final String dropDomainStmt : generateDropDomainStatements()) {
            jdbcTemplate.execute(dropDomainStmt);
        }
    }

    private List<String> generateDropConstraintStatements() throws SQLException {
        return jdbcTemplate.query("select RDB$RELATION_NAME, RDB$CONSTRAINT_NAME\n"
            + "from RDB$RELATION_CONSTRAINTS\n"
            + "where RDB$RELATION_NAME NOT LIKE 'RDB$%'\n"
            + "and RDB$CONSTRAINT_TYPE='FOREIGN KEY'", rs -> {
    ...
    private List<String> generateDropPackageStatements() throws SQLException {
        final List<String> packageNames = jdbcTemplate.queryForStringList("select RDB$PACKAGE_NAME as packageName\n"
            + "from RDB$PACKAGES\n"
            + "where (RDB$SYSTEM_FLAG is null or RDB$SYSTEM_FLAG = 0)");
    ...
    private List<String> generateDropProcedureStatements() throws SQLException {
        final List<String> procedureNames = jdbcTemplate.queryForStringList(
    ...
    private List<String> generateDropViewStatements() throws SQLException {
        final List<String> viewNames = jdbcTemplate.queryForStringList("select RDB$RELATION_NAME as viewName\n"
            + "from RDB$RELATIONS\n"
    ...
    private List<String> generateDropTriggerStatements() throws SQLException {
        final List<String> triggerNames = jdbcTemplate.queryForStringList("select RDB$TRIGGER_NAME as triggerName\n"
            + "from RDB$TRIGGERS\n"
            + "where (RDB$SYSTEM_FLAG is null or RDB$SYSTEM_FLAG = 0)\n");
    ...
    private List<String> generateDropFunctionStatements() throws SQLException {
        final List<String> functionNames = jdbcTemplate.queryForStringList("select RDB$FUNCTION_NAME as functionName\n"
            + "from RDB$FUNCTIONS\n"
            + "where (RDB$SYSTEM_FLAG is null or RDB$SYSTEM_FLAG = 0)");

        final String functionTypeName = database.getVersion().isAtLeast("3.0") ? "function" : "external function";
    ...
    private List<String> generateDropSequenceStatements() throws SQLException {
        final List<String> sequenceNames = jdbcTemplate.queryForStringList("select RDB$GENERATOR_NAME as sequenceName\n"
            + "from RDB$GENERATORS\n"
            + "where (RDB$SYSTEM_FLAG is null or RDB$SYSTEM_FLAG = 0)\n");
    ...
    private List<String> generateDropExceptionStatements() throws SQLException {
        final List<String> exceptionNames = jdbcTemplate.queryForStringList(
    ...
    private List<String> generateDropDomainStatements() throws SQLException {
        final List<String> domainNames = jdbcTemplate.queryForStringList("select RDB$FIELD_NAME as domainName\n"
            + "from RDB$FIELDS\n"
            + "where RDB$FIELD_NAME not starting with 'RDB$'\n"
            + "and (RDB$SYSTEM_FLAG is null or RDB$SYSTEM_FLAG = 0)\n");
    ...
    private List<String> generateDropStatements(final String objectType, final List<String> objectNames) {
        final List<String> statements = new ArrayList<>(objectNames.size());
    ...
    protected FirebirdTable[] doAllTables() throws SQLException {
        final List<String> tableNames = jdbcTemplate.queryForStringList("select RDB$RELATION_NAME as tableName\n"
            + "from RDB$RELATIONS\n"
            + "where RDB$VIEW_BLR is null\n"
            + "and (RDB$SYSTEM_FLAG is null or RDB$SYSTEM_FLAG = 0)");

        final FirebirdTable[] tables = new FirebirdTable[tableNames.size()];
    ...
    @Override
    public FirebirdTable getTable(final String tableName) {
