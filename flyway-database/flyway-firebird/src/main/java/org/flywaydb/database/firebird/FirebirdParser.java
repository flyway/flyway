...
public class FirebirdParser extends Parser {
    ...
    public FirebirdParser(final Configuration configuration, final ParsingContext parsingContext) {
    ...
    protected Token handleKeyword(final PeekingReader reader,
        ...
        if ("SET".equalsIgnoreCase(keyword)) {
            ...
            final String possiblyTerm = reader.peek(TERM_WITH_SPACES.length());
            if (TERM_WITH_SPACES.equalsIgnoreCase(possiblyTerm)) {
                reader.swallow(TERM_WITH_SPACES.length());
                final String newDelimiter = reader.readUntilExcluding(context.getDelimiter().getDelimiter());
